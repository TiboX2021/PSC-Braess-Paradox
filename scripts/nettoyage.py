...
def connected_stations(network: Network) -> np.ndarray:
    ...
    connections = np.array(
        network["metro_connections"]
        + network["rer_connections"]
...
def compute_useless_vertices(network: Network) -> List[int]:
    ...
    for end in ends:
        ...
        while (
            current not in whitelist
            and len(
                non_blacklisted_next_stations := next_stations[
                    np.isin(next_stations, blacklist + [last_station], invert=True)
                ]
            )
            < 2
        ):
            ...
            if len(non_blacklisted_next_stations) > 0:
                print(
                    "non blacklisted next",
                    non_blacklisted_next_stations,
