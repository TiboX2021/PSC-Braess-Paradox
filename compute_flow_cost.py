...
def compute_flow_cost(flow: np.ndarray):
    ...
    if __b_coeffs is None or __a_coeffs is None:
        __b_coeffs = np.array(edge_distances + [5000] * (total_len - len(edge_distances)))
        __a_coeffs = np.ones(total_len)

    # IMPROVEMENT : only add the b_coeff where there are people? Else this is just a constant cost...
    total_cost = np.sum(__a_coeffs * flow * flow + __b_coeffs * flow)  # En réalité il faudrait écrire ça comme ça !
    return total_cost
